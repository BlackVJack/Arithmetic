...
public class Leetcode_2594 {
    ...
    public long repairCars2(int[] ranks, int cars) {
        ...
        while (left < right) {
            ...
            int sum = compute(mid, ranks, cars);
            if (sum >= cars) {
                min = mid;
                right = mid - 1;
            }
        ...
        if (left == right) {
            int sum = compute(left, ranks, cars);
            if (sum >= cars) min = left;
        }
        return min;
    }

    public int compute(Long time, int[] ranks, int cars) {
        int sum = 0;
        for (int rank : ranks) {
            sum += Math.floor(Math.sqrt(time / rank));
            if (sum >= cars) break;
        }
        return sum;
    }
...
//        int cars2 = 52;
//        System.out.println(leetcode_2594.repairCars2(ranks2, cars2));
    }
